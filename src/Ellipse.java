...
public class Ellipse extends Shape{
	...
	public Ellipse(int _x , int _y, int w, int h , Color _c) {
		x = _x;
		y = _y;
		width = w;
		height = h;
...
//		if( ((Math.pow(_x-this.x, 2)/(this.width/2)) + (Math.pow(_y-this.y, 2)/(this.height/2))) <=1 )return true;
//		else return false;
//	}
...
//		super.Draw();
		Panel.g.setColor(c);
		Panel.g.drawOval(x, y, width, height);
...
//		super.editDuplicate(co, idx);
		Ellipse r = new Ellipse(x, y, width, height, co);
		r.Draw();
		Memory.shapes.add(r);
	}
	@Override
...
//		super.move(xS, yS, xE, yE, isFinal);
		...
		if(!isFinal){
			...
			if( (((XN-x)>=0 && (xE-xS)>=0)||((XN-x)<0 && (xE-xS)<0)) && ((YN-y)>=0 && (yE-yS)>=0)||((YN-y)<0 && (yE-yS)<0) ){
				r = new Ellipse((int)XN, (int)YN, width, height, c);
			}else{
	...
	//	super.move(xS, yS, xE, yE, isFinal);
		...
		if(!isFinal){
			Ellipse r ;
			r = new Ellipse(xN, yN, width, height, c);
			r.Draw();
		}else{
			x = xN;
			y = yN;
...
//		super.resize(xS, yS, xE, yE, isFinal);
		...
		if(isFinal){
			...
			int h1h = (int)(dY*2);
			int w1w = (int)(dX*2);
			Ellipse r = new Ellipse(x1x, y1y, w1w, h1h, c);
			r.Draw();
		}
	}
